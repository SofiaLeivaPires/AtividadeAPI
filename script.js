...
const botaoDescobrir = document.getElementById("botaoDescobrir");
...
botaoDescobrir.addEventListener("click", async () => {
 
  inputFilme.value = "";


  const urlDescobrir = `https://api.themoviedb.org/3/discover/movie?api_key=${apiKey}&language=pt-BR&sort_by=popularity.desc&page=1`;

  try {
    const resposta = await fetch(urlDescobrir);
    const dados = await resposta.json();
